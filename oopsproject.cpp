...
void book_add_modify(int, char *);
void book_add_modify(int);
...
public:
	...
	virtual void getnewaccountdata() = 0;
};
...
void book_add_modify(int id, char user[20])
{
	seller t;
	fstream f;
	char ans = 'y';
...
void deletecart(int id)
{
	...
	fstream f, q;
	f.open("cart.txt", ios::in);
	q.open("temp.txt", ios::out | ios::trunc);
	while (f.read((char *)&c, sizeof(c)))
	{
		if (c.getaccess() == no)
			if (c.getid() == id)
...
label:
	...
	{
	case 1:
		search_access(id, user);
		break;
	case 2:
		search_title(id, user);
		break;
	case 3:
		search_author(id, user);
		break;
	case 4:
		search_cost(id, user);
		break;
...
void search_access(int id, char user[20])
{
	...
	fstream f, r;
	f.open("books.txt", ios::in);
	r.open("results.txt", ios::out | ios::trunc);
	while (f.read((char *)&c, sizeof(c)))
	{
		if (c.getaccess() == no)
		{
